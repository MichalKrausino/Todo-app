...
export function TemplatesView({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <button onClick={onBack} className="flex items-center gap-1 text-sm font-medium text-accent-deep">
      ...
      <header className="rise">
        <TextEffect as="h1" per="char" preset="blur" className="display text-[2.1rem] font-semibold leading-tight">Šablony</TextEffect>
            ...
            return (
              <li key={item.id} className="bg-card px-4 py-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button className="px-1 text-xs text-ink-faint" onClick={() => void deleteItem(item.id)}>
                    Smazat
                  </button>
                </div>
              </li>
