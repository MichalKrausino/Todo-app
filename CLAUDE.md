...
vzoru tab baru iOS 26): pilulka pod ikonou letí pružinou, a když prst na
doku zůstane a táhne, odlepí se a jede s ním — pružina za prstem lehce
...
i „tohle jsi zvolil" tutéž modrou a nešlo je rozeznat. Kalendář v Plánu
jde o krok dál: vybraný den je inkoustový kroužek (jediná plná výplň na
obrazovce), dnešek modré písmo a pod čísly sloupky času v barvách
klientů — týden jako tichý graf, ne tečky. Otevření termínu
...
`stroke-dashoffset`), kalendář v Plánu je pás čísel se sloupky času na papíře, ne dlaždice v kartě,
...
- [x] **Plán = kalendář a agenda vybraného dne.** Dřív pás čtrnácti dnů s tečkami a pod ním sekce na každý den, na kterém něco leželo — až třicet karet pod sebou, „bez termínu" a ohlédnutí úplně dole; prázdný den nešel ani vybrat, natož na něj něco naplánovat, a týden ani měsíc nebyly nikde vidět. Teď (`UpcomingView`): (1) hlavička se souhrnem týdne vybraného dne („tento týden · 5 úkolů · ~4 h · 2 schůzky", jinde „týden od 14. 9."); (2) řádka chipů pro to, co není den — „Dnes" (jen mimo dnešek), „Bez termínu · N", „Týdenní ohlédnutí" (neděle/pondělí); (3) **kalendář** (`PlanKalendar`) je **týden jako tichý graf času**: stojí přímo na papíře (jediná karta na obrazovce je agenda pod ním) a pod každým číslem je **sloupek** — výška je naplánovaný čas (strop osm hodin, `plannedMinutes` úkolů + délka schůzek), **barvy jsou klienti**, šedá schůzka nebo úkol bez klienta; týden se tak čte „čtvrtek je V Bílém, pátek nabitý, víkend prázdný" (dvě verze předtím byly kalendář jako všude: tečka umí říct jen „něco tam je"). Jediná plná výplň je **inkoustový kroužek** vybraného dne, který mezi dny plyne (`layoutId`; audit kontrastu čte podklad z předků, proto kroužek dostane vlastní `bg-ink` po dojezdu, 400 ms, v klidu hned); dnešek je modré písmo a v hlavičce sloupců má místo zkratky štítek „dnes"; váha čísla říká, jestli na dni něco je. **Listuje se nativně**: pás tří stránek (minulá · tato · další) se `scroll-snap`, tah má setrvačnost prohlížeče, po dojetí (`scrollend`, záloha debounce) se pás v `useLayoutEffect` tiše přestaví na prostřední stránku a **výběr jde s sebou** (`posunVyber`: týden dopředu = tentýž den příští týden, měsíc = totéž číslo), takže agenda dole vždycky patří dni, který je vidět; šipky jen pro myš (`pointer-fine:`), „Dnes" vpravo v hlavičce jen mimo dnešek. Název měsíce vlevo je přepínač na celý měsíc (`todo.plan.rezim`, lokální volba; měsíc má tolik řádků, kolik chce nejdelší ze tří stránek pásu, aby tah neposkakoval a prázdný šestý řádek se nekreslil zbytečně). Nálož se počítá pro celé okno pásu (`oknoPasu`). **Karta dne z vybraného sloupce vyrůstá**: má ocásek (`rotate-45` čtverec nad kartou, `left` podle sloupce dne, přejíždí přechodem) a nadpis dne je uvnitř karty, ne nad ní; sloupky při vykreslení stránky vyrostou odspoda, každý sloupec o 30 ms později (`motion.span`, `height`), v klidu stojí; (4) **agenda vybraného dne v jedné kartě**: nadpis „Dnes / Pátek 11. září · 2 úkoly · ~2 h", schůzky (bez bloků „Todo"), úkoly (`DlouhySeznam` po 12), pod kartou tiché pole „Nový úkol na dnešek…" (parser + `dueDate` = vybraný den, toast) a „+ Vybrat z úkolů bez termínu · N". Minulý den ukáže, co se ten den dodělalo (`doneOn`). **Bez termínu je panel** (`BezTerminuSheet`): z chipu jen k nahlédnutí, z vybraného dne jako výběr — „Sem" pošle úkol na ten den (`dueDate`, `status: 'active'`, toast se „Zpět" vrací do inboxu); fronta se snímá při otevření, aby odeslaný řádek nepodtrhl další pod prstem. Plánuje se tak, jak se plánuje: napřed den, pak co na něj. Audit chování počítá `main li` v Plánu při 400 úkolech — vykreslí se jen agenda jednoho dne, ne celý měsíc.
