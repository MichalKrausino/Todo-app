...
- [x] Dnes jako jedna odpověď na „co teď?": **ranní návrh je jedna řádka s počtem**, rozhoduje se po jednom v panelu (`NavrhSheet`, stejný vzor jako triáž: velký název, důvod, „Přijmout na dnešek" / „Dnes ne", „Přijmout zbývající", „Zpět" vrací rozhodnutí na `ignored` a u přijatého i `scheduledFor` a blok v kalendáři). Dřív stál návrh jako seznam se dvěma kolečky na řádek a tytéž úkoly se o kus níž opakovaly v „po termínu" — obrazovka odpovídala dvakrát a pokaždé jinak. **Kalendář je sbalený na jednu řádku**: co běží, nebo nejbližší schůzka (po všech proběhlých ta poslední), celý den s volnými okny je na „Celý den · N schůzek"; vysvětlující věta o plusku zmizela (tlačítko má popisek).
