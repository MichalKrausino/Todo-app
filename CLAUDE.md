...
- [x] Fáze 3 — Google Calendar: edge funkce `calendar` (events/scheduleBlock/deleteBlock; tokeny v `public.google_tokens`, OAuth údaje v `public.google_oauth` — RLS bez policies, jen service role). Klientská vrstva `src/sync/calendar.ts` cachuje události do Dexie (`calendarEvents`, okno 14 dní), obnovuje po syncu a při návratu do popředí. Volná okna počítá `src/lib/freeSlot.ts` (server má kopii téže logiky — udržovat v souladu). Přijetí ranního návrhu zabere blok v kalendáři „Todo“ (délka = estimateMinutes ?? 60); zrušení naplánování/smazání úkolu blok uvolní. Follow-upy ze schůzek zatím nejsou.
