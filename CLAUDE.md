...
- [x] Triáž propadlých (`TriageSheet`): sekce „po termínu" umí narůst do stovek (změřeno 134 na roční hromádce) a jako seznam je to slepá ulička. Nadpis je proto akce — průchod po jednom se **žebříkem dnů**: Dnes → Zítra → Volnější den → Už neplatí, a oba odkladové dny nesou pod sebou konkrétní datum („ne 13. 9."), takže je vidět, kam to půjde. Dřív tu stálo jediné „Příští týden (pondělí)" — jedno tlačítko, jedno datum, takže sto propadlých úkolů skončilo na témž pondělí; to je tatáž zeď, jen o týden dál. „Volnější den" je nejbližší pracovní den s nejmenší zátěží do **sedmi dnů** (`src/lib/volnyDen.ts`, strop je úmyslný — odložit o měsíc není odložení) a počítá se **živě**: každý odložený úkol tam přibude, takže další stisk najde jiný den a hromádka se rozprostře (změřeno: tři stisky po sobě daly st 16., čt 17., pá 18.). Souhrn na konci vypisuje jen odpovědi s nenulovým počtem. Fronta se snímá při otevření, jinak by živý dotaz pod rukama přerovnával pořadí. Termín se posouvá stejně jako všude jinde (`scheduledFor`, a když ho úkol nemá, `dueDate`) — pevný termín se nikdy nepřepisuje potichu. „Už neplatí" nastaví `status: 'dropped'`, ne tombstone: úkol zmizí z otevřených seznamů, ale zahozená práce zůstane v datech. „Zpět" vrací i to. **Panel se jmenuje týmž slovem jako řádka, ze které se do něj ťuklo** (prop `nadpis` z `popisPropadlych`): jmenoval se „Po termínu" natvrdo, takže řádka poctivě hlásila „nestihnuto · 2" a o jedno ťuknutí dál stálo nad týmiž dvěma úkoly „Po termínu" — appka si v jednom gestu protiřečila (hlídá audit chování, oddíl 14, ověřeno vrácenou vadou). Tlačítko dole vlevo je **„Přeskočit"**, ne „Nechat být": není to odpověď — úkol nechá propadlý a jen posune frontu na další. „Nechat být" znělo jako rozhodnutí („tenhle už řešit nebudu"), tedy skoro jako „Už neplatí" o dvě řádky výš, a nešlo je od sebe poznat; sloveso říká přesně ten mechanismus a tvoří dvojici se „Zpět" vedle. **O odkládání se mluví jen tam, kde byl slib** (`src/lib/odkladani.ts`, čisté funkce s testy). `postponeCount` appka razítkuje odjakživa, ale mluvila o něm jen v signálech na Dnes a v nedělním ohlédnutí — tedy v rekapitulaci, nikdy ve chvíli, kdy člověk mačká „Volnější den" potřetí. Změřeno na 46 úkolech provozu (celá historie): úkoly bez odkladu se dodělaly z **81 %** (22 z 27), s jedním odkladem z **50 %** (7 ze 14) a **z pěti úkolů odložených dvakrát a víc ani jeden**. Napoprvé z toho vyšla značka „odloženo 5×" u každého odkládaného úkolu a byl to špatný závěr, protože **odklad není jedna věc**. Jsou nejmíň tři důvody, proč se termín posune: prokrastinace, nestihl jsem to, a „vím, že to budu muset udělat, ale ne teď" — úkol se zapíše, aby nezapadl, a posouvá se do doby, kdy na něj bude čas. První dva od sebe appka nerozezná a je to tak dobře (soudit ze záznamu v databázi, jestli byl člověk líný, nebo měl den navíc práce, je hazard); ten **třetí rozeznat jde a není to selhání** — je to funkční způsob, jak si věc připomínat, a to posouvání JE ta připomínka. Pozná se po termínu, přesně tím rozdílem, který appka **už používá nad seznamem propadlých** („po termínu" vs „nestihnuto"): termín je slib danému dni, naplánování je den, který si člověk vybral sám. Změřeno na týchž datech rozdělených podle termínu: z pěti úkolů odložených dvakrát a víc jich **čtyři žádný termín nemají**, takže by značka mířila hlavně na parkoviště — a `POSTPONE_THRESHOLD` v signálech na Dnes byl na tom ještě hůř, protože **oba** úkoly, které ho přetahují, jsou bez termínu: ten signál svítil VÝHRADNĚ na vědomě odloženou práci. Pravidlo je proto **mluví se jen o slibech** a ctí ho i `signals.ts`, jinak má appka na jednu věc dva názory. Odkládaný termín dostane v kartě triáže „· odloženo 5×" v `note-ink` (**práh 2**, z měření; signály zůstávají o jedna výš a ten rozdíl drží test: seznam, co se ukáže sám, má být vzácnější než věta u úkolu, na který se člověk zrovna dívá). Odkládaný úkol **bez** termínu značku nedostane — dostane **nabídku „Bez data"**, pátou odpověď v žebříku, která sundá termín i naplánování. Datum pryč, úkol ne: připomínat se nepřestane (Bez termínu v Plánu, ranní návrh, Vše), jen přestane lhát o tom, že něco propadlo, a nebude ho dokola honit triáž. **Status se přitom nemění na `inbox`** — odtamtud by ho po pár dnech vyhrabal signál „ležáky v inboxu" (`agingInbox` filtruje právě na `inbox`), tedy další nadávání za totéž. Nabídka se ukáže jen tam, kde se ten vzorec opravdu ukázal (`jeParkovany`), jinak by to byla pátá odpověď pro každého a žebřík by se rozpadl; „Zpět" ji vrací jako každou jinou odpověď. **Bilanci říká ohlédnutí, ne Dnes** (`vetaOOdkladani`): „Z úkolů s termínem, které jsi odložil aspoň dvakrát, se ti zatím dodělal žádný z pěti." — vlastní čísla, ne poučka, a jen z dost velkého vzorku (`MIN_VZOREK`, jinak je to náhoda, ne bilance); na dnešních datech je takových slibů **jediný**, takže věta správně mlčí. Číslovky se píšou slovem i s předložkou („ze čtyř", ale „z pěti"): „0 z 5" vypadá jako skóre zápasu. Hlídá to audit chování (oddíl 18, deset kontrol) a patnáct unit testů; každé ze tří nových pravidel je ověřené vrácenou vadou a každá shodí právě svoji kontrolu: značka bez ohledu na termín shodí ticho u parkovaného úkolu, nabídka bez ohledu na termín ji přidá i slibu a „Bez data", která sundá jen termín, nechá naplánování stát (k tomu čtyři unit testy).
