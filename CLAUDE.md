...
je `edge`** — schůzky a práce bez klienta. Kreslí se v Plánu pod číslem každého dne
v mřížce (v malém, 28 px) i přes celou šířku u vybraného dne, a na Dnes
pod hlavičkou. **V mřížce prázdný den značku nedostane**: na řádku přes
celou šířku platilo „graf s nulou má pořád osu" a prázdný den měl tichou
kolej, ale třicet kolejí vedle sebe je šedá tapeta a značka přestane
znamenat cokoli — v mřížce je osou sama mřížka. Na Dnes nahradil barevnou tečku před větou
...
- [x] **Fáze 11 — den má strop a appka to řekne včas** (`src/lib/kapacitaDne.ts`, čisté funkce s testy). Přetížený den uměla appka poznat odjakživa (`isOverloaded`), jenže **jen na obrazovce Dnes** — tedy v den, kdy už se s tím nedá nic dělat. Ve chvíli, kdy se práce na budoucí den SYPE, mlčela úplně. Změřeno na dvanácti dnech skutečného provozu: čtvrtek se **7 úkoly a 495 minutami dal jedno odškrtnutí**, pátek se 4 úkoly a 225 minutami nula, čtvrtek předtím se 3 úkoly nula — zatímco **obě soboty, na které se neplánovalo nic, daly po dvou**. Dny, které se naplní, jsou přesně ty, co spadnou; sedm úkolů z toho čtvrtka tam navíc leželo i o tři dny později. **Nezavádí se druhá míra vytíženosti**: strop je `PLNY_DEN_MIN`, tedy délka pruhu dne a zároveň pracovní doba (`WORK_START`–`WORK_END`), a protože pruh sčítá odhady úkolů PLUS schůzky, znamená přetečený pruh přesně „úkoly se nevejdou do zbytku pracovní doby" (`úkoly + schůzky > 8 h ⟺ úkoly > 8 h − schůzky`). Strop tedy nemá vlastní číslo, jen dává hlas tomu, co pruh odjakživa kreslí. **Tolerance 30 minut je záměr** — den přetečený o dvacet minut je normální den a signál, který svítí pořád, přestane být signál. **Appka nezakazuje, jen řekne a uhne**: úkol na plný den opravdu jde a zůstane tam, dokud s ním člověk sám nepohne; toast řekne výsledek („st 23. 9. má 10 h práce") a nabídne **Jinam**, což je TÝŽ volnější den, jaký volí triáž propadlých i ranní návrh (`volnejsiDen`) — hledá se od DALŠÍHO dne, protože ten vybraný je plný a jinak by se vrátil sám. Platí to na obou místech, kde se práce vědomě sype na den: pole „Nový úkol na…" v Plánu a „Sem" v panelu Bez termínu (tam toast nese **obě** cesty — „Jinam" i „Zpět", protože zrušit se musí dát i krok, po kterém appka něco namítla, jinak by z upozornění byla past). Minuty se sčítají z nálože obrazovky PLUS odhad nového úkolu: živý dotaz o něm ještě neví a čekat na překreslení by znamenalo hlásit strop o úkol později, tedy zase pozdě. **Srovnaly se tím tři různé představy o plném dni**, které appka měla na JEDNÉ obrazovce: agenda porovnávala holé `> PLNY_DEN_MIN` (bez tolerance) a svítila červeně na dni, kde pruh nad ní byl plný tak akorát, `DEFAULT_DAY_CAPACITY_MIN` byla šest hodin, takže pod titulkem Dnes stál pruh ze sedmi osmin plný a hned pod ním věta „na den je toho moc". Teď je strop jeden a je to ten, který je vidět. **V mřížce to nese ČÍSLO dne, ne pruh** (`note-ink`, polotučně): pruh je dlouhý jako čas a přes strop se natáhnout nemůže, takže den s osmi a den s třinácti hodinami kreslí totéž — a mřížka je přitom jediné místo, kde se den vybírá. Značka useknuté osy na konci pruhu zůstala jen pro pruh přes celou šířku; v buňce je pruh 28 px a týž pětipixelový proužek z něj zabere pětinu — změřeno na snímku ve čtyřnásobném zvětšení a čte se jako DALŠÍ KLIENT, ne jako „useknuto". Výběr a dnešek zůstávají nad tím: kde zrovna stojím, je vždycky důležitější než jak je tam nabito. Toast bere krátký popisek dne (`formatDayLabel`, „so 12. 9."), ne „sobota 12. září" — má `max-w-48` a delší věta se v něm ořízne. Hlídá to audit chování (oddíl 15, devět kontrol ověřených vrácenou vadou: mlčící strop shodí čtyři, věčně křičící tři, vypuštěný tón čísla jednu). **Barva čísla se čte proti TOKENU, ne proti jinému dni** — srovnání „plný den vs nějaký jiný" je planá kontrola: kdyby se tón nekreslil vůbec, vyšel by plný den jako `ink` a ten druhý jako víkendový `ink-soft`, tedy dvě různé barvy a kontrola projde, i když signál není. **STROP JE OSOBNÍ, ne nominální** (`src/lib/prutok.ts`, čisté funkce s testy). Osm hodin je poctivé číslo o hodinách, ale o téhle appce neříká nic: v ní nestojí celý den, jen práce, kterou si do ní člověk zapíše — schůzky, telefonáty a hašení jsou mimo. Změřeno na sedmnácti dnech provozu: **medián 120 min za den, průměr 97, nejlepší den 150**. Nominální strop byl tedy **víc než trojnásobek životního rekordu** — strážce, který se nemá jak ozvat: na dvanácti dnech plánování by spustil jediný, a to jen díky schůzkám. Ten čtvrtek s 495 minutami nebyl „o kousek přes", byl to **trojnásobek nejlepšího dne, jaký kdy byl**. Strop se proto počítá z vlastní historie (`osobniPrutok`/`stropZPrutoku`): okno 30 dní, **dobrý den** (`PERCENTIL` 0,8) krát `REZERVA` 1,5, zastropováno pracovní dobou a podložené `MIN_STROP_MIN`. **Ne z mediánu** — strop na obvyklém dni by se ozval skoro pokaždé a signál, který svítí pořád, není signál. Na skutečných datech vychází **180 min**, tedy strop, který se ozve na čtvrtku (495) i pátku (225) a mlčí na dni se 165 — přesně na těch dnech, co spadly. **Počítají se jen dny, ve kterých se něco dokončilo**: den bez odškrtnutí není den s nulovým průtokem, ale nejspíš den, kdy se appka neotevřela, a nuly by strop stáhly k zemi za každý volný týden. **Pod `MIN_DNU` (7) se strop nepočítá vůbec** — appka si o novém člověku nic nevymýšlí a drží se pracovní doby. Počítá se jen z MOJÍ práce (`mojeUkoly`): u sdíleného klienta odškrtává i kolega. **Číslo drží jeden hook** (`useOsobniStrop`) a berou si ho Plán i Dnes — kdyby si ho každá obrazovka počítala po svém, byl by plný den na jedné jinde než na druhé. **Celý pruh dne je od té doby STROP, ne osm hodin**: jinak by pruh a verdikt nad ním říkaly každý něco jiného. **Strop musí být v závislostech `useMemo`**, který z něj počítá značky do mřížky — přichází z živého dotazu, takže se po prvním vykreslení změní z pracovní doby na osobní, a bez něj by mřížka zůstala u verdiktu, který padl dřív, než appka toho člověka znala. **A hlavně: to číslo je vidět** — týdenní ohlédnutí říká „za den ti projde obvykle 1,5 h, v dobrý den 2 h (z 12 dnů práce), Plán proto hlídá 3 h na den". Bez téhle věty je strop číslo, které nikde nestojí, a takovému se nedá věřit; patří do ohlédnutí, ne na Dnes — je to číslo k zamyšlení, ne k popohánění. Hlídá to audit chování (oddíl 16, dvě kontroly): týž den se třemi hodinami práce projde bez historie a s osmi dny historie je přeplněný. Ověřeno vrácenou vadou — návrat k nominálnímu stropu tu druhou shodí. **Závislost `strop` v memu je správná z principu, ale audit ji nedokazuje**: ten závod se v něm spolehlivě netrefí, takže se na něj nelze spolehnout jako na pojistku.
