...
jde o krok dál: vybraný den je inkoustová pilulka (jediná plná výplň na
obrazovce), dnešek modré písmo, vytížení váha čísla a tečky uvnitř buňky. Otevření termínu
...
`stroke-dashoffset`), kalendář v Plánu je pás čísel na papíře, ne dlaždice v kartě,
...
- [x] **Plán = kalendář a agenda vybraného dne.** Dřív pás čtrnácti dnů s tečkami a pod ním sekce na každý den, na kterém něco leželo — až třicet karet pod sebou, „bez termínu" a ohlédnutí úplně dole; prázdný den nešel ani vybrat, natož na něj něco naplánovat, a týden ani měsíc nebyly nikde vidět. Teď (`UpcomingView`): (1) hlavička se souhrnem týdne vybraného dne („tento týden · 5 úkolů · ~4 h · 2 schůzky", jinde „týden od 14. 9."); (2) řádka chipů pro to, co není den — „Dnes" (jen mimo dnešek), „Bez termínu · N", „Týdenní ohlédnutí" (neděle/pondělí); (3) **kalendář** (`PlanKalendar`) stojí **přímo na papíře jako pás čísel**, ne v kartě — jediná karta na obrazovce je agenda pod ním (první verze byla kalendář z telefonu v bílé krabici: modré šipky, modrý den i modré tečky si konkurovaly). Jediná plná výplň je **inkoustová pilulka** vybraného dne, která mezi dny plyne (`layoutId`, týž vzor jako pilulka pod záložkou doku; audit kontrastu čte podklad z předků, proto vybraná buňka dostane vlastní `bg-ink` po dojezdu, 400 ms, v klidu hned); dnešek je modré písmo (`accent-deep`) a místo zkratky dne má štítek „dnes"; vytížení dělá **váha písma a až tři tiché tečky uvnitř buňky** (`ink-faint`, od pěti položek `danger`), ne semafor pod ní; minulé dny `ink-faint`, prázdné budoucí `ink-soft`. Název měsíce vlevo je přepínač na celý měsíc (`todo.plan.rezim`, lokální volba), vpravo tiché šipky a „Dnes" (jen mimo dnešek). Listuje se šipkami i švihnutím (pointer events, práh 48 px, svislý tah patří rolování přes `touch-action: pan-y`) a **listování bere výběr s sebou** (`posunVyber`: týden dopředu = tentýž den příští týden, měsíc = totéž číslo), takže agenda dole vždycky patří dni, který je vidět; (4) **agenda vybraného dne v jedné kartě**: nadpis „Dnes / Pátek 11. září · 2 úkoly · ~2 h", schůzky (bez bloků „Todo"), úkoly (`DlouhySeznam` po 12), pod kartou tiché pole „Nový úkol na dnešek…" (parser + `dueDate` = vybraný den, toast) a „+ Vybrat z úkolů bez termínu · N". Minulý den ukáže, co se ten den dodělalo (`doneOn`). **Bez termínu je panel** (`BezTerminuSheet`): z chipu jen k nahlédnutí, z vybraného dne jako výběr — „Sem" pošle úkol na ten den (`dueDate`, `status: 'active'`, toast se „Zpět" vrací do inboxu); fronta se snímá při otevření, aby odeslaný řádek nepodtrhl další pod prstem. Plánuje se tak, jak se plánuje: napřed den, pak co na něj. Audit chování počítá `main li` v Plánu při 400 úkolech — vykreslí se jen agenda jednoho dne, ne celý měsíc.
